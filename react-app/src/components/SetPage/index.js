...
export default function SetPage() {
  ...
  const questions = useSelector((state) =>
    Object.values(state.questions.allQuestions)
  );
  const setQuestions = questions.filter(
    (question) => question.set_id === Number(set_id)
  );
        ...
        <div className="set-page-inner-div">
          <div className="set-page-header">
            ...
            <span className="set-description">
              ...
              {!sessionUser && folder.is_public === "yes" && (
                <button
                  className="log_out_button nav-button"
                  onClick={() => history.push(`/folders/${folder_id}`)}
                >
                  Return to Folder
                </button>
              )}
            ...
            {sessionUser?.id === set?.user_id && (
              <div className="set-btns-div">
                <OpenModalButton
                  id="edit-set-btn"
                  buttonText="Edit Set"
                  modalComponent={
                    <EditSet folderId={folder.id} set={set ? set : null} />
                  }
                />
                <OpenModalButton
                  id="delete-set-btn"
                  ...
                  modalComponent={
                    <DeleteSetModal folderId={folder.id} setId={set.id} />
                  }
                />
                <button
                  className="log_out_button nav-button"
                  onClick={() => history.push(`/folders/${folder.id}`)}
                >
                  Back to folder
                </button>
          ...
          <p className="set-text-small">
            {set?.questions.length && set?.questions.length !== 0 ? (
              `Total Questions in Set: ${set.questions.length}`
            ) : (
              <span className="no-sets">
                There are no questions in this set
              </span>
            )}
          </p>
          <h3 className="set-heading">Hover over card to reveal answer</h3>
            ...
            <div className="card-carousel-container">
              ...
              <div className="card-flip-carosel-div">
                <div className={`card-flip-carosel ${shuffle}`}>
                  {setQuestions[questionArrayIndex]?.description.length > 1000
                    ? (qlen = "long")
                    : setQuestions[questionArrayIndex]?.description.length > 200
                    ? (qlen = "med")
                    : (qlen = "short")}
                  {/* {setQuestions[questionArrayIndex]?.description.length>50?qlen="med":qlen="short"} */}
                  <div className={`card-text question ${qlen}`}>
                    {setQuestions[questionArrayIndex] &&
                      setQuestions[questionArrayIndex].description}
                  </div>
                  {setQuestions[questionArrayIndex]?.answer.length > 1000
                    ? (alen = "long")
                    : setQuestions[questionArrayIndex]?.answer.length > 200
                    ? (alen = "med")
                    : (alen = "short")}
                  ...
                  <div className={`card-text answer ${alen}`}>
                    Answer:{" "}
                    {setQuestions[questionArrayIndex] &&
                      setQuestions[questionArrayIndex].answer}
                  </div>
                </div>
              </div>
              {/* &nbsp; */}
              <span className="carousel-btns">
                <button
                  ...
                  onClick={() => {
                    ...
                    questionArrayIndex <= 0
                      ? setQuestionArrayIndex(setQuestions.length - 1)
                ...
                <button
                  ...
                  onClick={() => {
                    // console.log("value of questionArrayIndex:", questionArrayIndex);
                    shuffleCard();
                    questionArrayIndex >= setQuestions.length - 1
          ...
          {sessionUser?.id === set?.user_id && (
            <div className="add-question-btn">
              <OpenModalButton
                ...
                modalComponent={
                  <CreateQuestionModal folderId={folder.id} setId={set.id} />
                }
              />
            </div>
          )}
